...
const Buscar = () => {
  ...
  const handleDificultadChange = (e) => {
    ...
    setIsFirstVisit(false);
    buscarRutina(search, selectedDificultad).then(({ data }) =>
      setRutinas(data)
    );
  };
      ...
      <div className="search-header">
        ...
        >
          ...
          {dificultades.map((dificultad) => (
            <option value={dificultad} key={dificultad}>
              {dificultad}
            </option>
          ))}
        </select>
        ...
        <button
          className="default-btn-2"
          onClick={eliminarFiltro}
          style={{
            visibility:
              dificultad !== "" || search !== "" ? "visible" : "hidden",
          }}
        >
      ...
      <div className="temp-content-body mt-10">
        {rutinas.length > 0 ? (
          ...
          <NotRutins
            ...
            mensaje={
              isFirstVisit
                ? "Comienza buscando una rutina o selecciona una categoría."
                : "No encontramos ninguna rutina con ese nombre. Intenta con otro título."
            }
            showButton={false}
          />
        )}
